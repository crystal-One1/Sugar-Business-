...
// Root-level logger to debug incoming requests and set robust CORS headers
...
app.get('/api/app-data', (req, res) => {
  ...
  res.json({
    ...
    users: db.users.map((u: any) => ({
      ...
      favoriteProductIds: u.favoriteProductIds || [],
...
app.post('/api/users/login', (req, res) => {
  ...
  res.json({
    ...
    user: {
      ...
      followedProductIds: user.followedProductIds || [],
...
// 13b. E-Commerce: Add Product Review
app.post('/api/shop/products/review', (req, res) => {
  const { productId, userName, userPhone, rating, comment } = req.body;
  if (!productId || !userName || !rating || !comment) {
    return res.status(400).json({ success: false, message: "بيانات التقييم غير كاملة." });
  }
  ...
  res.json({
    success: true,
    message: "تم تسجيل تقييمك ورأيك بنجاح بالمنتج. شكراً لمشاركتك شريكنا العزيز!",
    products: db.products
  });
});
...
// 27b. User: Toggle Favorite Product (Wishlist)
app.post('/api/users/toggle-favorite', (req, res) => {
  ...
  user.favoriteProductIds = user.favoriteProductIds || [];
  const idx = user.favoriteProductIds.indexOf(productId);
  if (idx >= 0) {
    user.favoriteProductIds.splice(idx, 1);
  } else {
    user.favoriteProductIds.push(productId);
  }

  writeDB(db);
  res.json({ success: true, favoriteProductIds: user.favoriteProductIds });
});
